...
fn test_iter_match_name_parent_grandparent() {
    ...
    // Debug
    let debug_ctx = ContextIter::new(&pool);
    println!("Grandparent test context:");
    for (i, item) in debug_ctx.enumerate() {
        println!("  context[{}]: {:?}", i, std::str::from_utf8(item).unwrap_or("invalid utf8"));
    }
