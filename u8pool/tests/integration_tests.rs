...
fn test_buffer_overflow() {
    ...
    assert!(u8pool.push(b"hello").is_ok());
    assert!(u8pool.push(b"world").is_ok());

    // Try to add more data than fits in the remaining space
    assert!(u8pool
        .push(b"this_is_a_very_long_string_that_should_definitely_not_fit_in_the_remaining_space_because_it_is_way_too_long_and_exceeds_the_buffer_capacity_by_a_lot")
        .is_err());
}
